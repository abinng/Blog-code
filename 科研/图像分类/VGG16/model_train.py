# ==============================================================================
# Python Script: VGG16 Model Training and Evaluation on FashionMNIST
# Author: [Original Author/Your Name] # 请在此处填写作者姓名
# Date: 2025-12-21 # 已更新为当前日期
# Description:
# This script implements the training and validation process for a VGG16
...
#    using Matplotlib.
# The VGG16 model definition is assumed to be in a separate 'model.py' file.
# ==============================================================================
...
from model import VGG16  # 从自定义的model.py文件中导入VGG16模型类。
...
def train_model_process(model, train_dataloader, val_dataloader, num_epochs):
    ...
    # 保存最佳模型的参数到指定路径。
    torch.save(best_model_wts, "best_model_VGG16.pth")
...
def matplot_acc_loss(train_process):
    ...
    plt.title("Training and Validation Loss (VGG16)")  # 设置子图标题。
    ...
    plt.legend()  # 显示图例。
    plt.title("Training and Validation Accuracy (VGG16)")  # 设置子图标题。

    plt.tight_layout()  # 自动调整子图参数，使之填充整个图像区域。
    # 保存图形，确保在plt.show()之前调用。文件名已更新为实际使用的批次大小和epoch数量。
    plt.savefig("fashion_VGG-16_28bs_20ep.png")
    plt.show()  # 显示图形。
...
if __name__ == "__main__":
    ...
    # 利用VGG16模型、数据加载器和指定的epoch数量（20）进行模型训练。
    # 训练过程的指标将存储在train_process DataFrame中。
    train_process = train_model_process(
        model_vgg16, train_dataloader, val_dataloader, num_epochs=20
    )
