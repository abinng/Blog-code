# ==============================================================================
# Python Script: GoogLeNet Model Training and Evaluation on Custom Image Dataset
# Author: [Original Author/Your Name]
# Date: [Current Date]
# Description:
# This script implements the training and validation process for a GoogLeNet
# convolutional neural network on a custom image dataset (e.g., cat/dog).
# It includes:
# 1. Data loading, preprocessing (resizing, tensor conversion, and normalization)
#    from a directory structure, and splitting into training and validation sets.
...
# ------------------------------------------------------------------------------
import copy  # 导入copy模块，用于深度复制模型参数，以便保存最佳模型。
import time  # 导入time模块，用于计算训练过程的耗时。

import torch  # 导入PyTorch库，它是构建深度学习模型的核心。
from torchvision.datasets import (
    ImageFolder,
)  # 从torchvision导入ImageFolder，用于加载按文件夹结构组织的图像数据集。
from torchvision import (
    transforms,
)  # 从torchvision导入transforms模块，用于数据预处理和增强。
import torch.utils.data as Data  # 从torch.utils.data导入Data模块，用于处理数据集和数据加载器。
import numpy as np  # 导入numpy库，用于数值计算（在此脚本中未直接使用，但常用于数据处理）。
import matplotlib.pyplot as plt  # 导入matplotlib.pyplot模块，用于数据可视化。
from model import (
    GoogLeNet,
    Inception,
)  # 从自定义的model.py文件中导入GoogLeNet模型类和其核心Inception模块。
import torch.nn as nn  # 从PyTorch导入神经网络模块，包含损失函数等。
import pandas as pd  # 导入pandas库，用于创建和处理数据表格，这里用于保存训练过程的指标。
...
def train_val_data_process():
    """
    处理训练和验证数据集：加载自定义图像数据集，进行预处理，并创建数据加载器。

    Returns:
        tuple: 包含训练数据加载器和验证数据加载器。
    """
    # 定义自定义训练数据集的根路径。
    # 假设训练集图片按照 'data/train/类别名/图片名.jpg' 的结构存放。
    ROOT_TRAIN = r"data/train"

    # 定义图像归一化参数。
    # 这些均值和标准差应根据整个训练数据集计算得出，以确保数据一致性。
    normalize = transforms.Normalize(
        [0.48613475, 0.45297343, 0.41533014],  # 每个通道的均值（R, G, B）。
        [0.06911735, 0.06530484, 0.06676254],
    )  # 每个通道的标准差（R, G, B）。
    ...
    train_transform = transforms.Compose(
        [
            ...
            # --- 图片增强 ---
            transforms.ToTensor(),  # 将PIL Image或numpy.ndarray转换为Tensor，并自动归一化到[0.0, 1.0]。
            normalize,  # 对Tensor进行标准化，使其均值为0，标准差为1。
        ]
    )
    # 使用ImageFolder加载数据集。ImageFolder会自动根据文件夹名分配类别标签。
    ...
    train_dataloader = Data.DataLoader(
        ...
        num_workers=32, # 用于数据加载的子进程数量，提高加载效率。
        pin_memory=True,    # 这个参数告诉 DataLoader 在将数据从 CPU 传输到 GPU 之前，将其存储在可分页的内存中。这可以加速数据传输。
    )  
...
def train_model_process(model, train_dataloader, val_dataloader, num_epochs):
    ...
    for epoch in range(num_epochs):
        print("Epoch {}/{}".format(epoch, num_epochs - 1))  # 打印当前epoch进度。
        print("-" * 10)  # 打印分隔线。
        ...
        with torch.no_grad():  # 禁用梯度计算，提高验证效率。
        ...
        print(
            "训练和验证耗费的时间{:.0f}m{:.0f}s".format(time_use // 60, time_use % 60)
        )

    # 训练结束后，加载在验证集上表现最佳的模型参数。
    model.load_state_dict(best_model_wts)
    # 保存最佳模型的参数到指定路径。
    # 注意：请确保路径 '' 存在或修改为合适的路径。
    torch.save(best_model_wts, "best_model_GoogLeNet_cat_dog.pth")
...
def matplot_acc_loss(train_process):
    ...
    plt.savefig("cat_dog_GoogLeNet_512bs_50ep.png")
    plt.show()  # 显示图形。
...
if __name__ == "__main__":
    ...
    # 注意：这里变量名GoogLeNet与类名相同，在实际编程中建议使用不同的变量名，例如 `model_googlenet`。
    ...
    # 利用GoogLeNet模型、数据加载器和指定的epoch数量进行模型训练。
    # 训练过程的指标将存储在train_process DataFrame中。
    # num_epochs=50 表示训练50个周期。
    train_process = train_model_process(
        model_googlenet, train_dataloader, val_dataloader, num_epochs=50
    )
