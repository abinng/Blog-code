# ==============================================================================
# Python Script: ResNet18 Model Training and Evaluation on FashionMNIST
# Author: [Original Author/Your Name] # 请在此处填写作者姓名
# Date: 2025-12-27 # 已更新为当前日期
# Description:
# This script implements the training and validation process for a ResNet18
...
#    using Matplotlib.
# The ResNet18 model definition and its building block (Residual) are assumed
# to be in a separate 'model.py' file.
# ==============================================================================

# 1. Import Necessary Libraries
# ------------------------------------------------------------------------------
import copy  # 用于深度复制模型状态，以便保存最佳模型。
...
from model import Residual, ResNet18  # 从自定义的model.py文件中导入Residual块和ResNet18模型类。
...
def train_val_data_process():
    ...
    train_data = FashionMNIST(
        ...
        transform=transforms.Compose(
            [  # 定义数据转换操作序列。
                transforms.Resize(
                    size=224
                ),  # 将图片尺寸调整为224x224，以适应ResNet18的输入要求。
    ...
    train_dataloader = Data.DataLoader(
        ...
        batch_size=32,  # 每个批次的样本数量，此处设置为32。
...
def train_model_process(model, train_dataloader, val_dataloader, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        # 注意：在验证阶段，通常使用 `with torch.no_grad():` 上下文管理器来禁用梯度计算，
        # 以节省内存并加速计算。此处的代码省略了该上下文管理器，
        # 但 `model.eval()` 已经将模型设置为评估模式，会关闭Dropout并固定BatchNorm层的统计量。
        with torch.no_grad(): # 推荐在验证阶段使用此上下文管理器以节省计算资源。
    ...
    # 保存最佳模型的参数到指定路径。
    # 路径 "C:/Users/86159/Desktop/ResNet18/best_model.pth" 是一个Windows系统下的绝对路径，
    # 在实际部署或跨平台使用时，建议使用相对路径或更通用的路径管理方式。
    torch.save(best_model_wts, "best_model_ResNet18.pth") # 修改为相对路径，更通用。
...
def matplot_acc_loss(train_process):
    ...
    plt.title("Training and Validation Loss (ResNet18)")  # 设置子图标题。
    ...
    plt.legend()  # 显示图例。
    plt.title("Training and Validation Accuracy (ResNet18)")  # 设置子图标题。

    plt.tight_layout()  # 自动调整子图参数，使之填充整个图像区域。
    # 保存图形，确保在plt.show()之前调用。文件名已更新为实际使用的批次大小和epoch数量。
    plt.savefig("fashion_ResNet18_32bs_20ep.png")
    plt.show()  # 显示图形。
...
if __name__ == "__main__":
    ...
    """
    # 实例化ResNet18模型。
    # ResNet18模型通常以Residual块作为其基本构建单元。
    model_resnet18 = ResNet18(Residual)
    print("ResNet18 Model Initialized.")

    # 加载并处理数据集，获取训练和验证数据加载器。
    # 注意：这里的train_data和val_data实际上是数据加载器（DataLoader）对象。
    train_dataloader, val_dataloader = train_val_data_process()
    print("Dataset Loaded and Processed.")

    # 利用ResNet18模型、数据加载器和指定的epoch数量（20）进行模型训练。
    # 训练过程的指标将存储在train_process DataFrame中。
    train_process = train_model_process(
        model_resnet18, train_dataloader, val_dataloader, num_epochs=20
    )
