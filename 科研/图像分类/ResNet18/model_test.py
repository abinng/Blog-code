# ==============================================================================
# Python Script: ResNet18 Model Testing on FashionMNIST
# Author: [Original Author/Your Name] # 请在此处填写作者姓名
# Date: 2025-12-27 # 已更新为当前日期
# Description:
# This script is designed to evaluate a pre-trained ResNet18 model on the
...
def test_model_process(model, test_dataloader):
    ...
    with torch.no_grad():
        # 遍历测试数据加载器中的每一个mini-batch（这里由于batch_size=1，实际上是单个样本）。
    ...
    # 计算测试准确率。
    # .double().item() 将张量转换为Python浮点数，以获得精确的数值。
...
if __name__ == "__main__":
    ...
    # 请确保此文件存在于脚本运行的相同目录或指定路径中。
    # 注意：这里加载的文件名是 'best_model.pth'，请确保与训练脚本保存的文件名一致。
    ...
    with torch.no_grad():
        ...
        for i, (b_x, b_y) in enumerate(test_dataloader):
            ...
            # 打印预测值和真实值。
            print(
                f"Sample {i + 1}: Predicted: {classes[result]} | True: {classes[label]}"
            )
